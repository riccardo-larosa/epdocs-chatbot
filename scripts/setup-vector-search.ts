...
class VectorSearchSetup {
  ...
  async runSetup() {
    ...
    // Test vector search (only if documents exist)
    try {
      await this.testVectorSearch(collectionName);
    } catch (error) {
      console.log('ℹ️  Vector search test skipped (no documents yet)');
    }
    ...
    console.log('\n📝 Next Steps:');
    console.log('1. Add your documentation files to ./docs/');
    console.log('2. Run: npm run update-docs');
    console.log('3. Test the chatbot: npm run dev');
...
async function main() {
  ...
  const setup = new VectorSearchSetup(config);
